...
class HOT24Dataset(BaseDataset):
    """
    ...
    def __init__(self):
        super().__init__()
        self.base_path = self.env_settings.hot24_path
        # self.sequence_list = self._get_sequence_list()
        self.sequence_list = self.get_hot_sequence_list()
    ...
    def _construct_sequence(self, sequence_name):
        ...
        modality = sequence_name.split('-')[0]
        class_name = sequence_name.split('-')[1]
        anno_path = '{}/{}/{}/groundtruth_rect.txt'.format(self.base_path, 'HSI-'+modality_mapping[modality]+'-FalseColor',class_name)
        ...
        target_visible = np.ones(len(ground_truth_rect), dtype=bool)

        frames_path = '{}/{}/{}'.format(self.base_path, 'HSI-'+modality_mapping[modality]+'-FalseColor',class_name)
        ...
        return Sequence(sequence_name, frames_list, 'hot24_large', ground_truth_rect.reshape(-1, 4),
                        object_class=target_class, target_visible=target_visible)

    def __len__(self):
        return len(self.sequence_list)
    
    def get_hot_sequence_list(self):
        modality_prefixes = {
        'HSI-VIS-FalseColor': 'vis',
        'HSI-NIR-FalseColor': 'nir',
        'HSI-RedNIR-FalseColor': 'rednir'}
        sequence_list = []
        for folder_name, prefix in modality_prefixes.items():
            folder_path = os.path.join(self.base_path, folder_name)
            sequences = [f"{prefix}-{seq}" for seq in os.listdir(folder_path) if os.path.isdir(os.path.join(folder_path, seq))]
            sequence_list = sequence_list + sequences
        return sequence_list
    
    def _get_sequence_list(self):
        sequence_list = ['stone-NIR','stone-RGB','stone-VIS']
