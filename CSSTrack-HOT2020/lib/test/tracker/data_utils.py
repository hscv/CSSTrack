...
class Preprocessor(object):
    def __init__(self):
        ...
        self.tmp_mean = self.mean_arr[:16]
        self.tmp_std  = self.std_arr[:16]
        self.mean = torch.tensor(self.tmp_mean).view((1, 16, 1, 1)).cuda()
        self.std = torch.tensor(self.tmp_std).view((1, 16, 1, 1)).cuda()
