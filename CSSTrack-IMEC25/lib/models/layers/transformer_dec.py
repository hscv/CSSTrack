...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory, 
        ...
        output = tgt
        intermediate = []
        for kk in range(memory[0].size()[0]):
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt_all, memory,
        ...
        tgt_kv_arr = []
        memory_arr = []
        band_split_group = tgt_all[0].size()[1]
        assert band_split_group == 4

        tgt_res_arr = []
        for kk in range(band_split_group):
            tgt_q_arr.append(tgt_all[0][:,kk:kk+1,:])
            tgt_kv_arr.append(tgt_all[1][:,kk:kk+1,:])
            memory_arr.append(memory[320*kk:320*(kk+1),:,:])

        for kk in range(band_split_group):
            tgt_kv = tgt_kv_arr[kk]
            tgt_q = tgt_q_arr[kk]
            memory = memory_arr[kk]
            q = tgt_q
            k = tgt = tgt_kv
            ...
            tgt = self.norm3(tgt)
            tgt_res_arr.append(tgt)

        tgt = torch.cat(tgt_res_arr, dim=1)
