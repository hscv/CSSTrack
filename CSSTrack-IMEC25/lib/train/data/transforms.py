...
class Normalize(TransformBase):
    ...
    def transform_image(self, image):
        mean_tmp_arr = [self.mean[i%3] for i in range(30)]
        std_tmp_arr = [self.std[i%3] for i in range(30)]
        return tvisf.normalize(image, mean_tmp_arr[:25], std_tmp_arr[:25], self.inplace)
