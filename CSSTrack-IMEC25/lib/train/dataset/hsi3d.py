...
class HSI3D(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, vid_ids=None, split=None, data_fraction=None):
        ...
        rootDir = env_settings().imec25_dir
        self.HOTDataRootDir = rootDir

        self.cur_modalName = split
        self.sequence_list = os.listdir(self.HOTDataRootDir)
        self.sequence_list .sort()
        self.all_img_info_dic = self.getAllImageInfo()
        if data_fraction is not None:
            self.sequence_list = random.sample(self.sequence_list, int(len(self.sequence_list)*data_fraction))

    def getImgs(self, videoname, suffix='.tif'):
    ...
    def getAllImageInfo(self):
        ...
        for videoname in self.sequence_list:
            ...
            hsi_imgs = self.getImgs(os.path.join(self.HOTDataRootDir, videoname, 'HSI'), '.tif')
            gt_arr_hsi = self.getGTBox(os.path.join(self.HOTDataRootDir, videoname, 'HSI/groundtruth_rect.txt'))

            assert len(hsi_imgs) == len(gt_arr_hsi)
    ...
    def _get_sequence_path(self, seq_id):
        seq_name_hsi = self.sequence_list[seq_id]
        seq_path_hsi = os.path.join(self.root, seq_name_hsi)
        return seq_path_hsi
    ...
    def _get_rgb_hsi_frame_path(self, seq_path, frame_id):
        seq_paths = sorted(glob(os.path.join(seq_path, '*.tif')))
        seq_paths_rgb = seq_paths[frame_id]
        seq_paths_hsi = seq_paths_rgb.replace("FalseColor", "3D").replace("tif", "npy")
        return (seq_paths_rgb, seq_paths_hsi)


    def X2Cube(self, img, cellSize=5):
    ...
    def _get_frames(self, seq_id, frame_ids):
        ...
        for frame_id in frame_ids:
            frame_name = '%04d.tif' % (frame_id+1)
            image_path = os.path.join(self.HOTDataRootDir, self.sequence_list[seq_id], 'HSI', frame_name)
            hsi_image = self.get_image_loader(image_path, cellSize=5)
            frame_list.append(hsi_image)
        return frame_list
