...
class Preprocessor(object):
    def __init__(self):
        self.mean_arr = [0.485, 0.456, 0.406] * 10
        self.std_arr = [0.229, 0.224, 0.225] * 10
        self.tmp_mean = self.mean_arr[:25]
        self.tmp_std  = self.std_arr[:25]
        self.mean = torch.tensor(self.tmp_mean).view((1, 25, 1, 1)).cuda()
        self.std = torch.tensor(self.tmp_std).view((1, 25, 1, 1)).cuda()
